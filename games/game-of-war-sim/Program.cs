...
public class Game
{
    ...
    public Game(string player1Name, string player2Name, bool silent = false)
    {
        _silent = silent; //store it
    ...
    public int Start()
    {
        ...
        DeclareWinner();
        return rounds; //return round count
    }
    ...
    public void DeclareWinner()
    {
        ...
        //_rounds.PrintAll();  <------------uncomment to print round data
    }
    ...
    private int PlayRound(int rounds)
    {
        ...
        if(!_silent) Console.WriteLine($"\n--------------Round:[{rounds}]----------------------");
        if (!_silent) Console.WriteLine($"Player 1 card: {card_player1.ToString()}");
        if (!_silent) Console.WriteLine($"Player 2 card: {card_player2.ToString()}\n");
        ...
        if (card_player1.Value > card_player2.Value)
        {
            ...
            if (!_silent) Console.WriteLine("Player 1 wins the round!");
        }
        else if (card_player1.Value < card_player2.Value)
        {
            ...
            if (!_silent) Console.WriteLine("Player 2 wins the round!");
        }
        else //war situation
        {
            if (!_silent) Console.WriteLine("WAR!!\n");
            ...
            while (!warOver)
            {
                ...
                {
                    if (!_silent) Console.WriteLine($"{player2.Name} wins the war - {player1.Name} doesn't have enough cards!");
                ...
                {
                    if (!_silent) Console.WriteLine($"{player1.Name} wins the war - {player2.Name} doesn't have enough cards!");
                ...
                if (!_silent) Console.WriteLine("Each player draws 3 cards and adds to the pot...");
                for (int i = 0; i < 3; i++)
                {
                ...
                if (!_silent) Console.WriteLine($"{player1.Name} war card: {warCard1}");
                if (!_silent) Console.WriteLine($"{player2.Name} war card: {warCard2}");

                if (warCard1.Value > warCard2.Value)
                {
                    winner = player1.Name;
                    if (!_silent) Console.WriteLine($"{player1.Name} wins the war and takes {JackPot.Count} cards from the pot!");
                    foreach (Card c in JackPot) player1.AddCard(c);
                    potSize = JackPot.Count;
                    warOver = true;
                }
                ...
                {
                    winner = player2.Name;
                    if (!_silent) Console.WriteLine($"{player2.Name} wins the war and takes {JackPot.Count} cards from the pot!");
                    foreach (Card c in JackPot) player2.AddCard(c);
                    potSize = JackPot.Count;
                    warOver = true;
                }
                ...
                {
                    if (!_silent) Console.WriteLine("WAR AGAIN!!"); //tied, run another war
                }
            }
        }
        if (!_silent) Console.WriteLine("----------------------------------------------");
...
public class RoundLinkedList
{
    ...
    public void ExportToJson(string filename)
    {
        ...
        while (current != null)
        {
            ...
            sb.Append($"{{\"roundNumber\":{r.RoundNumber},");
            sb.Append($"\"player1Card\":\"{r.Player1Card.Rank}\",");
            sb.Append($"\"player2Card\":\"{r.Player2Card.Rank}\",");
            sb.Append($"\"winner\":\"{r.Winner}\",");
            sb.Append($"\"warCount\":{r.WarCount},");
...
public class Program
{
    ...
    {
        // delete old file before starting so we get a clean run
        if (System.IO.File.Exists("rounds.json"))
            System.IO.File.Delete("rounds.json");
        ...
        {
            //Console.WriteLine("$\n======= GAME {i + 1} ========");
            Game game = new Game("Aidan", "Computer", silent: true); //(no per-round output)
            totalRounds += game.Start();
        }

        Console.WriteLine($"\n========= Results ===========");
        Console.WriteLine($"Games played: {numGames}");
        Console.WriteLine($"Total rounds: {totalRounds}");
        Console.WriteLine($"Average rounds per game: {totalRounds / numGames}");
